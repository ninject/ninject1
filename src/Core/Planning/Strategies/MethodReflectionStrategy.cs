...
namespace Ninject.Core.Planning.Strategies
{
	...
	public class MethodReflectionStrategy : ReflectionStrategyBase<MethodInfo>
	{
		...
		/// <returns></returns>
		protected override MethodInfo[] GetMembers(IBinding binding, Type type, BindingFlags flags)
		{
		...
		protected override void AddInjectionDirective(IBinding binding, Type type, IActivationPlan plan, MethodInfo member)
		{
			...
			// Use it to create a new injector that can inject values into the method.
			IMethodInjector injector = injectorFactory.Create(member);

			// Create a new directive that will hold the injection information.
			MethodInjectionDirective directive = new MethodInjectionDirective(member, injector);
